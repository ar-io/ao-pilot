...
import { __ as __2, always as always2, append as append2, assoc as assoc2, concat as concat2, defaultTo as defaultTo2, ifElse as ifElse2, pipe as pipe2, prop as prop2 } from "ramda";
...
import { __ as __3, always as always3, append as append3, assoc as assoc3, concat as concat3, defaultTo as defaultTo3, ifElse as ifElse3, pipe as pipe3, prop as prop4 } from "ramda";
...
function buildTagsWith2() {
  return (ctx) => {
    return of(ctx).map(prop4("tags")).map(defaultTo3([])).map(removeTagsByNameMaybeValue("Data-Protocol", "ao")).map(removeTagsByNameMaybeValue("Variant")).map(removeTagsByNameMaybeValue("Type")).map(removeTagsByNameMaybeValue("Module")).map(removeTagsByNameMaybeValue("Scheduler")).map(removeTagsByNameMaybeValue("SDK")).map(concat3(__3, [
      { name: "Data-Protocol", value: "ao" },
      { name: "Variant", value: "ao.TN.1" },
...
function monitorWith(env) {
  ...
  return ({ process: process2, signer }) => of({ id: process2, signer }).chain(verifyProcess).chain(uploadMonitor).map((ctx) => ctx.monitorId).bimap(errFrom, identity4).toPromise();
}
...
function unmonitorWith(env) {
  ...
  return ({ process: process2, signer }) => of({ id: process2, signer }).chain(verifyProcess).chain(uploadUnmonitor).map((ctx) => ctx.monitorId).bimap(errFrom, identity5).toPromise();
}
