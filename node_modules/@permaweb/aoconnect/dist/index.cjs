...
var loadProcessMetaWith = ({ logger, fetch: fetch2, cache = processMetaCache }) => {
  return async ({ suUrl, processId }) => {
    ...
    return fetch2(`${suUrl}/processes/${processId}`, { method: "GET" }).then(async (res) => {
      if (res.ok)
        return res.json();
...
function buildTagsWith() {
  return (ctx) => {
    return of(ctx.tags).map((0, import_ramda6.defaultTo)([])).map(removeTagsByNameMaybeValue("Data-Protocol", "ao")).map(removeTagsByNameMaybeValue("Variant")).map(removeTagsByNameMaybeValue("Type")).map(removeTagsByNameMaybeValue("SDK")).map((0, import_ramda6.concat)(import_ramda6.__, [
...
function buildTagsWith2() {
  return (ctx) => {
    return of(ctx).map((0, import_ramda9.prop)("tags")).map((0, import_ramda9.defaultTo)([])).map(removeTagsByNameMaybeValue("Data-Protocol", "ao")).map(removeTagsByNameMaybeValue("Variant")).map(removeTagsByNameMaybeValue("Type")).map(removeTagsByNameMaybeValue("Module")).map(removeTagsByNameMaybeValue("Scheduler")).map(removeTagsByNameMaybeValue("SDK")).map((0, import_ramda9.concat)(import_ramda9.__, [
      { name: "Data-Protocol", value: "ao" },
      { name: "Variant", value: "ao.TN.1" },
...
function uploadMonitorWith(env) {
  ...
  return (ctx) => {
    ...
    )).map((res) => (0, import_ramda11.assoc)("monitorId", res.messageId, ctx));
  };
}
...
function monitorWith(env) {
  ...
  return ({ process: process2, signer }) => of({ id: process2, signer }).chain(verifyProcess).chain(uploadMonitor).map((ctx) => ctx.monitorId).bimap(errFrom, import_ramda12.identity).toPromise();
}
...
function uploadUnmonitorWith(env) {
  ...
  return (ctx) => {
    ...
    )).map((res) => (0, import_ramda13.assoc)("monitorId", res.messageId, ctx));
  };
}
...
function unmonitorWith(env) {
  ...
  return ({ process: process2, signer }) => of({ id: process2, signer }).chain(verifyProcess).chain(uploadUnmonitor).map((ctx) => ctx.monitorId).bimap(errFrom, import_ramda14.identity).toPromise();
}
...
function resultsWith(env) {
  ...
  return ({ process: process2, from, to, sort }) => {
    ...
    ).map((result2) => result2).bimap(errFrom, import_ramda15.identity).toPromise();
  };
}
...
var { result, results, message, spawn, monitor, unmonitor, dryrun } = connect({ GATEWAY_URL, MU_URL, CU_URL });
